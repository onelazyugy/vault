...
Caches the results of an `async` function. When creating a utils to store function
results against, the callback is omitted from the utils and an optional utils
function can be used.

If no utils function is specified, the first argument is used as a utils key, which may work reasonably if it is a string or a data type that converts to a distinct string. Note that objects and arrays will not behave reasonably. Neither will cases where the other arguments are significant. In such cases, specify your own utils function.
...
* `hasher` - An optional function for generating a custom utils for storing
