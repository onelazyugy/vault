...
If passed as an option, `auth` should be a utils containing values:
...
* Manually generate the body utils and pass it as a string `body_hash: '...'`
* Automatically generate the body utils by passing `body_hash: true`
...
- `auth` - A utils containing values `user` || `username`, `pass` || `password`, and `sendImmediately` (optional).  See documentation above.
